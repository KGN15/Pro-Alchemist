...
export default function LoginPage() {
  ...
  async function sendOtp(e: React.FormEvent) {
    ...
    try {
      ...
      setStep("otp");
      setMsg(data.message || "কোড ইমেইলে পাঠানো হয়েছে।");
    } catch {
  ...
  <div className="relative w-full max-w-md overflow-hidden rounded-[2.5rem] border border-white/5 bg-black/40 p-10 backdrop-blur-2xl shadow-2xl">
    <div className="relative z-10 text-center">
      <h1 className="text-3xl font-black tracking-tight text-white">লগইন <span className="text-red-600">করুন</span></h1>
      <p className="mt-2 text-sm text-zinc-400">
        ইমেইলে ওটিপি পাবেন — পাসওয়ার্ড লাগবে না।
      </p>
    </div>
      ...
      <form onSubmit={verify} className="relative z-10 mt-10 space-y-6 text-center">
        ...
        {msg && (
          <p className="text-sm font-medium text-emerald-400" role="status">
            ✅ {msg} Inbox চেক করুন (স্প্যাম ফোল্ডারও)। 
          </p>
        )}
    ...
    <div className="mt-10 border-t border-white/5 pt-8 text-center">
      <p className="text-sm text-zinc-500">
        অ্যাকাউন্ট নেই?{" "}
        <Link href="/register" className="font-bold text-red-500 hover:text-red-400">
          নিবন্ধন করুন
        </Link>
      </p>
